...
import { fetchRevenueStatsShorts } from '@/lib/api';
import { useTranslation } from 'react-i18next';
import { formatWithDecimalsNumber } from '@/utils/formatDecmials';

export default function RevenueStatsShorts() {
  ...
  useEffect(() => {
    const loadRevenueStats = async () => {
      try {
        const data = await fetchRevenueStatsShorts();
        setRevenueData(data);
      } catch (error) {
  ...
  return (
    <div className="rounded-lg p-4"
      style={{
        background: 'url(/assets/line-bg.c23fa9a4.png) no-repeat',
      ...
      <div className="flex items-center mb-2">
        ...
        <>
          {revenueData.packageIcon ? (
            <img src={`/vip-icons/vip${revenueData.vipLevel}.png`} className="w-[50px] h-[50px]" />
          ) : (
            <img src={`/vip-icons/vip0.png`} className="w-[50px] h-[50px] opacity-[.1]" />
          )}

          <div className='flex flex-col ml-2'>
            <span className="text-[12px] font-sora font-medium">{t('revenue.currentlevel')}:</span>
            <span className="text-base font-bold text-white">VIP{revenueData.vipLevel}</span>
          </div>
        </>
        }
      </div>
