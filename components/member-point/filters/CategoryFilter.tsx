...
export default function CategoryFilterDown({ onFilterChange }: CategoryFilterProps) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute w-auto p-[.625rem_.9375rem] pt-1 mt-[10px] rounded-[0.5rem] bg-[#181818] shadow-lg text-left z-[1013] right-0">
          {filters.map((key) => (
            <div
              key={key}
              onClick={() => handleSelectItem(key)}
              className="h-[40px] w-full flex flex-row justify-start items-center text-left border-b-[0.0625rem] border-[#282828] cursor-pointer text-white font-sora text-[14px] font-medium leading-[1.0625rem] whitespace-nowrap"
            >
